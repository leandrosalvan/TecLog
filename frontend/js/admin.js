// TecLog+ — Painel do dono (clientes, planos, suspensão, exclusão, senhas)
...
async function carregar() {
  ...
  document.getElementById("resumo-clientes").textContent =
    data.clientes.length + (data.clientes.length === 1 ? " cliente" : " clientes");

  if (data.clientes.length === 0) {
    ul.innerHTML = '<li class="empty">Nenhum cliente ainda. Crie o primeiro acima.</li>';
    return;
  }
...
// Auto-preenche o limite ao escolher o plano (criação)
...
document.getElementById("form-redef").addEventListener("submit", async (e) => {
  e.preventDefault();
  const r = await fetch("/api/admin/redefinir-senha", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
  ...
  const d = await r.json().catch(() => ({}));
  if (r.ok) {
    document.getElementById("form-redef").reset();
    msg("msg-redef", "Senha de " + d.nome + " (" + d.tipo + ") redefinida! ✅", "ok");
  } else {
    msg("msg-redef", d.erro || "Erro ao redefinir.", "erro");
  }
});
